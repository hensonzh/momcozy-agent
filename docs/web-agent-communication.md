...
| `forwardedProps` | 当前传空对象 | 后端支持从这里读取 locale、timezone、message_sent_at、profile 等扩展上下文 |
...
| `message` | 兜底文本字段 |
| `state` | 可包含 `locale`、`timezone`、`message_sent_at`、profile 等 |
...
- 前端目前只显式传 `locale`。
- `timezone` 默认由后端使用 `America/Los_Angeles`。
- `message_sent_at` 如果前端不传，由后端按 timezone 实时生成。
...
```json
{
  ...
  "locale": "zh-CN",
  "message_sent_at": "2026-05-12T11:00:00.000Z",
  "idempotency_key": "uuid"
}
```

后端当前只强校验 `ticket` 必须是 object，其余字段用于模拟请求语义。
...
5. 会话连续性由后端 session + Responses API `previous_response_id` 维护，前端只需要稳定传 `threadId`。
