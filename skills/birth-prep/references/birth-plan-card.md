...
在调用 `ui_form_create` 前，先输出一句简短引导：
...
[
  ...
  },
  {
    ...
    "type": "multi_select",
    "required": true,
    "options": ["干预前先解释", "先征求同意", "包含伴侣/支持人", "语言简单清楚", "需要翻译或语言支持", "其他"],
...
[
  ...
  },
  {
    ...
    "type": "multi_select",
    "required": false,
    "options": ["skin-to-skin", "尽早哺乳", "希望先沟通再使用配方奶", "宝宝护理事项先解释", "未确定", "其他"]
  },
  {
    "id": "if_plans_change",
    "label": "如果计划变化，什么最重要",
...
> 你可以在产检或入院前把这张卡给医生/护士看；其中问题部分建议提前和医院确认。
...
- `overview` 只放孕周/预产期、分娩方式、支持人这类能帮助沟通的短信息，不要扩展成详细背景档案。
- `communication`、`pain_relief`、`baby_after_birth`、`if_plans_change` 都使用短句数组，每组最多 3 条。
- `questions_for_hospital` 最多 3 条，只放需要医院确认的问题，不要编造医院政策。
...
如果 `confirmed_form_data.birth_path` 是 `计划剖宫产`，仍然使用同一个 `birth_plan_card` schema，不新增字段或分区。
...
- 把重点放入已有字段：
  ...
  - `baby_after_birth`：skin-to-skin 时间、喂养偏好、宝宝护理沟通。
  - `medical_notes`：仅放用户明确提供的过敏、医生说明或医院限制。
  - `questions_for_hospital`：陪同、拍照、术后接触宝宝、喂养和住院流程等需要医院确认的问题。
- 不提供手术指导，不替代医生说明。
...
- 不确定内容放入 `questions_for_hospital`，不要编造医院政策。
