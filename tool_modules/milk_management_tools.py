...
MILK_MANAGEMENT_FUNCTION_TOOLS: dict[str, FunctionToolDefinition] = {
    ...
    "infant_growth_evaluate": _function_tool(
        ...
        {
            "user_id": USER_ID,
            "infant_id": _nullable({"type": "integer", "description": "Optional infant id. Pass null to evaluate the user's default infant."}),
            "as_of_time": _nullable({"type": "string", "description": "Optional ISO-8601 evaluation time."}),
        },
    ),
    "milk_plan_preview": _function_tool(
        ...
        "Generate an increase, maintain, or decrease milk plan draft. No database writes; user confirmation is required before apply.",
        {
            ...
            "options": _nullable(
                {
                    **JSON_OBJECT_STRING,
                    "description": "Free-form JSON object encoded as a string. May include prepared_assessment/prepared_growth_assessment from prior milk_assessment_evaluate/infant_growth_evaluate calls to avoid repeated evaluation, plus observed_persistent_abnormal or medical_confirmation_confirmed when confirmed by the user.",
                }
            ),
        },
    ),
    ...
    "milk_plan_target_validate": _function_tool(
        "milk_plan_target_validate",
        "Validate an explicit increase, maintain, or decrease milk target against current milk assessment and reference bounds. For decrease, delta_ml is the desired daily reduction and must be between 50 ml/day and current daily milk. No side effects.",
        {
            "user_id": USER_ID,
            "plan_type": {"type": "string", "enum": ["increase_milk", "maintain_milk", "decrease_milk"]},
    ...
    "milk_plan_validate": _function_tool(
        "milk_plan_validate",
        "Validate a full milk plan draft or edited saved plan before apply/update, including plan-type boundaries: increase must add pumping tasks and keep adjacent pumping intervals within 5h; decrease cannot exceed current milk or current frequency; maintain cannot change pumping task count. No side effects.",
        {
            "user_id": USER_ID,
    ...
    "milk_calendar_adjustment_preview": _function_tool(
        ...
        "Preview calendar changes for a new user event such as school pickup or a meeting. No database writes; user confirmation is required before apply.",
        {
            ...
            "content": {"type": "string"},
            "item_type": _nullable({"type": "string", "enum": ["吸奶", "亲喂", "自定义"], "description": "Classify the inserted event. Use 自定义 for meals, outings, meetings, school pickup, etc."}),
            "plan_id": _nullable({"type": "integer"}),
        },
    ),
    "milk_calendar_adjustment_apply": _function_tool(
        "milk_calendar_adjustment_apply",
        "Apply a user-confirmed calendar adjustment proposal. The proposal must include the inserted user event plus any task updates. Has side effects; call only after explicit confirmation.",
        {
            "user_id": USER_ID,
            "target_date": ISO_DATE,
